...
"SELECT title, SUM(pledges.amount) FROM projects JOIN pledges ON projects.id = project_id GROUP BY title ORDER BY title;"
end
...
"SELECT projects.title, SUM(pledges.amount) - projects.funding_goal FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY title HAVING SUM(amount) >= funding_goal;"
end
